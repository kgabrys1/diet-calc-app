...
export function calculateResults({
  ...
  const cd =
    weight !== targetWeight
      ? (CONSTANTS.humanFatCalorificValue * avgMassDiff) /
        CONSTANTS.timeGranulation
      : 0;
  const baseCi = tee + cd;
...
function getMassLossFactor(reductionIntensity: string): number {
  switch (reductionIntensity) {
    case "low":
      return CONSTANTS.massLossFactorLow;
    case "med":
      return CONSTANTS.massLossFactorMedium;
    case "high":
      return CONSTANTS.massLossFactorHigh;
    default:
      throw new Error(
        `Unsupported reductionIntensity: [${reductionIntensity}]`
      );
  }
}
...
const getTargetWeeks = (
  weight: number,
  targetWeight: number,
  massLossFactor: number
) => Math.abs(Math.round(-Math.log(weight / targetWeight) / massLossFactor));

const round = (val: number, n: number = 0) =>
...
const massReductionFunc = (
  weeks: number,
  weight: number,
  massLossFactor: number
) => massLossFactor * Math.pow(1 + massLossFactor, weeks - 1) * weight;
