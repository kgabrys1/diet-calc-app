...
export default function Input({ id, type, unit, value, children, tooltipText, onChange }: Readonly<InputProps>): React.ReactNode {
            ...
            {isGender &&
                <div id={id} className="flex space-x-2">
                    <div>
                        ...
                        />
                        <label htmlFor="maleGenderRadio" className="ms-1 font-medium text-white">Male</label>
                    </div>
                    <div>
                        <input
                        ...
                        />
                        <label htmlFor="femaleGenderRadio" className="ms-1 font-medium text-white">Female</label>
                    </div>
                </div>
            }
                ...
                <div id={id} className="flex space-x-2">
                    <div>
                        <input
                            ...
                            onChange={(event) => { onChange(id, event.target.value) }}
                            className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
                        <label htmlFor="reductionIntensityLow" className="ms-1 font-medium text-white">Low</label>
                    </div>
                    <div>
                        <input
                            ...
                            className="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
                        />
                        <label htmlFor="reductionIntensityMedium" className="ms-1 font-mediumtext-white">Medium</label>
                    </div>
                    <div>
                        <input
                            ...
                            className="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
                        />
                        <label htmlFor="reductionIntensityHigh" className="ms-1 font-medium text-white">High</label>
                    </div>
                </div>
            }
            ...
            {!isGender && !isReductionIntensity &&
                <div className="relative flex flex-wrap items-stretch w-64">
                    <input
                        ...
                        min={isPal ? 1.0 : 0}
                        onChange={(event) => { onChange(id, event.target.value) }}
                        className={`relative bg-gray-900 m-0 block flex-auto ${unit ? 'rounded-s' : 'rounded'} border border-solid bg-clip-padding px-2 py-[0.17rem] text-sm font-normal leading-[1.5] text-surface outline-none transition duration-200 ease-in-out focus:z-[3] focus:border-primary focus:shadow-inset focus:outline-none motion-reduce:transition-none border-white/10 text-white`}
